...
6. Check if `$PROJECT_ROOT/.understand-anything/meta.json` exists. If it does, read `gitCommitHash` and treat it as `$LAST_ANALYSIS_REF` (an opaque ref string, not necessarily a Git hash).
6.5. Check if `$PROJECT_ROOT/.understand-anything/fingerprints.json` exists. If it does, note that fingerprint-based incremental comparison is available.
7. **Decision logic:**
   ...
   | `--review` flag + existing graph + `$VCS_TYPE=git` + unchanged analysis ref | Skip to Phase 6 (review-only — reuse existing assembled graph) |
   | Existing graph + `$VCS_TYPE=git` + unchanged analysis ref | Ask the user: "The graph is up to date at this ref. Would you like to: **(a)** run a full rebuild (`--full`), **(b)** run the LLM graph reviewer (`--review`), or **(c)** do nothing?" Then follow their choice. If they pick (c), STOP. |
   | Existing graph + `$VCS_TYPE=git` + changed files | Incremental update (re-analyze changed files only) |
   | Existing graph + `$VCS_TYPE in {svn, none}` + no fingerprints baseline | Full analysis (all phases) |
   | Existing graph + `$VCS_TYPE in {svn, none}` + fingerprints baseline | Run fingerprint comparison to decide between `SKIP`, incremental update, or full rebuild |
   ...
   **Git incremental path:**
   ```bash
   git diff $LAST_ANALYSIS_REF..HEAD --name-only
   ```
   If this returns no files, report "Graph is up to date" and STOP.

   **SVN / no-VCS incremental path:** Write the compare input file:
   ```bash
   cat > $PROJECT_ROOT/.understand-anything/tmp/compare-input.json <<EOF
   {
     "projectRoot": "$PROJECT_ROOT",
     "analysisRef": "$CURRENT_ANALYSIS_REF"
   }
   EOF
   ```

   Then invoke the bundled comparison script (located next to this SKILL.md):
   ```bash
   node <SKILL_DIR>/compare-fingerprints.mjs \
     $PROJECT_ROOT/.understand-anything/tmp/compare-input.json
   ```

   Read `$PROJECT_ROOT/.understand-anything/tmp/update-decision.json`:
   - If `action = "SKIP"`: report "Graph is structurally up to date" and STOP.
   - If `action = "FULL_UPDATE"`: switch to a full rebuild for the current run.
   - If `action = "PARTIAL_UPDATE"` or `"ARCHITECTURE_UPDATE"`: proceed with incremental update using the generated `changed-files.txt`.
...
- **Git path:**
  ```bash
  git diff $LAST_ANALYSIS_REF..HEAD --name-only > $PROJECT_ROOT/.understand-anything/tmp/changed-files.txt
  ```

- **SVN / no-VCS path:** the file is already generated by `compare-fingerprints.mjs` at:
  ```bash
  $PROJECT_ROOT/.understand-anything/tmp/changed-files.txt
  ```
  Read `$PROJECT_ROOT/.understand-anything/tmp/update-decision.json` first:
  - If `action = "SKIP"`: report "Graph is structurally up to date" and STOP.
  - If `action = "FULL_UPDATE"`: abandon the incremental path and switch to a full rebuild for this run.
  - If `action = "PARTIAL_UPDATE"` or `"ARCHITECTURE_UPDATE"`: continue below.
...
This produces a `batches.json` that contains only batches with changed files, but neighborMap entries still reference unchanged files (with their full-graph batchIndex) so cross-batch edges remain emittable. The `changed-files.txt` file may include deleted paths; `compute-batches.mjs` naturally ignores paths absent from `scan-result.json`, while the prune step below still uses the full list to remove stale nodes from the existing graph.

Then dispatch file-analyzer subagents per the same template as the full path.

After batches complete:
1. Remove old nodes whose `filePath` matches any path in `changed-files.txt` from the existing graph (this includes deleted files)
...
```json
{
  ...
  "project": {
    ...
    "gitCommitHash": "<analysis ref from Phase 0>"
  },
...
2. **Generate structural fingerprints baseline.** This creates the basis for future automatic incremental updates and **must succeed before `meta.json` is written** — otherwise the next run sees a fresh analysis ref with no fingerprints to compare against, classifies every file as STRUCTURAL, and escalates to `FULL_UPDATE` on every subsequent manual incremental run (issue #152).
   ...
   cat > $PROJECT_ROOT/.understand-anything/intermediate/fingerprint-input.json <<EOF
   {
     ...
     "analysisRef": "<current analysis ref>"
   }
   EOF
   ```
...
3. Write metadata to `$PROJECT_ROOT/.understand-anything/meta.json` (only after step 2 succeeded):
   ```json
   {
     ...
     "gitCommitHash": "<analysis ref>",
     "version": "1.0.0",
     "analyzedFiles": <number of files analyzed>
   }
   ```
   Keep the field name `gitCommitHash` for backward compatibility, but store the opaque `$CURRENT_ANALYSIS_REF` value there in all VCS modes.
